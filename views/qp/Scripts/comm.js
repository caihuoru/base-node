...
var colors = ["0000ff", "009944", "b38850", "ff00ff", "ff0000", "00fff6", "00ff5a", "aeff00", "ffd800", "ff8400"];
// var colors = function getUserTypeColor() {
//     let color = ['#555555']
//     $.ajax({
//         type: 'GET',
//         url: qp_url + 'admin/Account/UserTypeColor?token=' + getStorage('token'),
//         dataType: 'json',
//         success: function (res) {
//             console.log()
//             if (res.status == 1) {
//                 for (let key in res.data) {
//                     color.push(res.data[key])
//                 }
//             }
//         }
//     })
...
(function ($) {
    OpeIframeWindow = function (url, mtitle, opt, isDiv) {
        ...
        if (isDiv)
            ...
            mDialog = $("<div id='edit_div'><iframe style='height:99%;width:99%' id='edit_iframe' src='" + url + "' scrolling='auto' frameborder ='0'/></div>").dialog(mopt);// scrolling='no'
        }
        this.Close = function () {
...
function openWindow(url, width, height) {
    ...
    var openthiswin = window.open(url, "welcomes", 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,left=' + left + ',top=' + top + ',width=' + width + ',height=' + height);
    if (openthiswin != null) openthiswin.focus();
}
...
//异步提交事件
function AjaxSubmit(action, data, fun, fname) {
    $.ajax({
        type: 'POST',
        url: action,
        dataType: 'json',
        ...
        success: function (rsp) {
            ...
            if (result.IsOk) {
                if (result.Data == null || result.Data == "") {
                    ...
                    fun(jsondata, fname, result.Total);
                }
            } else {
                if (result.Url != null && result.Url != "") {
                    window.open(result.Url, "frm_main_content");
                } else {
                    alert(result.Msg);
                }
            }
...
function SelectAll(obj, itemName) {
    $("[name=" + itemName + "]").prop("checked", obj.checked);
}
...
// 处理layui的选中事件
function GetSelectedDatas(data) {
    console.log(data)
    var array = [];
...
function GetSelectedDatas(data, key) {
    console.log(data)
    var array = [];
    for (var i = 0, j = data.length; i < j; i++) {
...
function GetCheckBoxByNameOfValues(name) {
    var array = [];
...
function GetCheckListCheckedSum(chkName) {
    var j = 0;
    $("input[name='" + chkName + "']:checked").each(function (i, item) {
        j = j | parseInt($(item).val());
    });
    return j;
}

function UploadImg(fileId, action, imgId, op) {
    var request = null;
...
function UploadFile(fileId, action, txtID) {
    var request = null;
    if (window.XMLHttpRequest) {
        request = new XMLHttpRequest();
    } else if (window.ActiveXObject) {
...
function CalVersion2(v) {
    var rValue = 0;
    var verArray = v.split('.');
    rValue = (parseInt(verArray[0]) << 24) | (parseInt(verArray[1]) << 16) | (parseInt(verArray[2]) << 8) | parseInt(verArray[3]);
    return rValue;
}
...
// 获取小数点后两位
function toFixed2(n) {
    return parseFloat(n.toString().match(/^\d*(\.?\d{0,2})/g)[0])
}
...
var Time = {
    today: function () {
        ...
        var resData = {
            ...
            EndDate: getDateString(date, 'endTime'),
            NowDate: getDateString()
        }
        return resData
    },
    yesterday: function () {
        ...
        var resData = {
            StartDate: getDateString(date),
            EndDate: getDateString(date, 'endTime'),
            NowDate: getDateString(new Date(new Date().getTime() - 86400000))
        }
        return resData
    },
    thisWeek: function () {
        var date = new Date();
        date = new Date(date.getTime() - 86400000 * (date.getDay() - 1))
        date.setHours(0);
        date.setMinutes(0);
        date.setSeconds(0);
        date.setMilliseconds(0);
        var resData = {
            StartDate: getDateString(date),
            EndDate: getDateString(date.getTime() + 86400000 * (7 - date.getDay()), 'endTime'),
        }
        return resData
    },
    lastWeek: function () {
        var date = new Date(new Date().getTime() - 86400000 * 7);
        date = new Date(date.getTime() - 86400000 * (date.getDay() - 1))
        date.setHours(0);
        date.setMinutes(0);
        date.setSeconds(0);
    ...
    monthFirstDay: function () {
        ...
        ;
        var resData = {
            firstDay: getDateString(new Date(y, m, 1)),
            lastDay: getDateString(new Date(y, m + 1, 0))
        }
        return resData
    }
}
...
$(function () {
    ...
    //     }
    // }, 500)
    $(document).bind('DOMNodeInserted', '.layui-table tbody', function (e) {
        if ($('.layui-table')) {
            if ($('.layui-table tbody tr').length < 1) {
                $('body').addClass('nodata')
            } else {
    ...
    $('.btn-minimize').click(function (e) {
        e.preventDefault();
        var $target = $(this).parents('.box-inner').find('.box-content');
